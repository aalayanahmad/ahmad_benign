...
def benign_events_list():
    ...
    list_to_execute = []
    potentially_active_ues_in_the_next_slot = []
    ...
    for lambda_value in LIST_OF_LAMBDAS:
      ...
      counter = 1
      if (lambda_value > 1):
        potentially_active_ues_in_the_next_slot = [ue_temp for ue_temp in ue_selected if ue_state[ue_temp + "_D"] >= slot_time] 
        ue_selected.clear()
        ue_selected = potentially_active_ues_in_the_next_slot.copy()
